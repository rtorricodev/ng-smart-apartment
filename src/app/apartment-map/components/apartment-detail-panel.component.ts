  ...
  template: `
    <div class="fixed w-2/6 h-36">
      <div class=" w-full h-1/3 bg-blue-600 flex justify-center items-center">
        <p (click)="emitBackEvent()" class="text-white cursor-pointer ">
          Back to Results
        </p>
      </div>
      <div
        class=" w-full h-2/3 bg-blue-500 flex justify-between items-center px-5 top-16"
      >
        <div>
          <p class="text-white">
            ...
            <span class="mx-2">
              <a
                [href]="selectedApartment ? selectedApartment.url : ''"
                target="_blank"
              >
    ...
    <div class="fixed w-2/6 h-28 mt-36 flex overflow-scroll justify-center">
      <ng-container
        *ngFor="let photo of selectedApartment ? selectedApartment.photos : []"
      >
        <img
          [src]="photo"
          alt="image of apartment"
          class="rounded-sm w-auto mx-1 my-2"
        />
      </ng-container>
    </div>
    ...
    <mat-tab-group class="fixed w-2/6 h-4/6 top-64 ">
      <mat-tab label="Plans" class=" overflow-scroll">
        <div class="w-full flex justify-center flex-wrap ">
          ...
          <ng-container
            *ngFor="
              let floor of selectedApartment ? selectedApartment.floorPlans : []
            "
          >
            <div
              class="w-11/12 h-16 bg-white rounded-md shadow-md m-3 flex justify-between items-center"
            >
              <p class="mx-3">{{ floor.bed }} bed, {{ floor.bath }} bath</p>
              <p class="mx-3">No connections</p>
              <p class="mx-3 text-blue-800 font-light text-lg">
                $ {{ floor.price }} - $ {{ floor.priceMax }}
              </p>
            </div>
          </ng-container>
        </div>
      ...
      <mat-tab label="Overview">
        ...
        <div class="w-full h-[9rem] flex justify-center flex-wrap">
          <ng-container
            *ngFor="
              let amenities of selectedApartment
                ? selectedApartment.highValueAmenities
                : []
            "
          >
            <div
              class="w-5/12 h-12 bg-white rounded-md shadow-md mx-3 my-2 flex items-center justify-center"
            >
              <p>{{ amenities }}</p>
            </div>
          </ng-container>
        ...
        <div class="w-full h-[12rem] flex justify-center flex-wrap">
          <ng-container
            *ngFor="
              let amenities of selectedApartment
                ? selectedApartment.propertyAmenities
                : []
            "
          >
            <div
        ...
        <div class="w-full h-[12rem] flex justify-center flex-wrap">
          <ng-container
            *ngFor="
              let unitAmenities of selectedApartment
                ? selectedApartment.unitAmenities
                : []
            "
          >
            <div
